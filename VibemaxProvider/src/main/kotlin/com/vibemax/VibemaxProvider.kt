...
class VibemaxProvider : MainAPI() {
    ...
    private fun parseHtmlCards(document: Document): List<SearchResponse> {
        ...
        return cards.mapNotNull { card ->
            try {
                val title = card.attr("aria-label").ifEmpty { return@mapNotNull null }
                ...
                val tmdbId = Regex("""(\d+)""").find(href)?.groupValues?.get(1) ?: return@mapNotNull null
        ...
        try {
            ...
            for (season in 1..numSeasons.coerceAtMost(10)) {
                ...
                for (ep in 1..episodesPerSeason) {
                    episodes.add(
                        newEpisode("$tmdbId|tv|$season|$ep") {
